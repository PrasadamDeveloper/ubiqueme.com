<template>
  <div class="fixed inset-0 z-50 flex items-end justify-center p-0 bg-black/40 md:items-center md:p-6"
    @click.self="$emit('dismiss')">

    <!-- Sheet -->
    <div
      class="relative w-full max-w-lg bg-white rounded-3xl rounded-b-none max-h-[90vh] flex flex-col shadow-xl animate-ios-sheet md:rounded-b-3xl md:max-w-2xl lg:max-w-3xl md:max-h-[85vh] md:shadow-2xl md:overflow-hidden">
      ...
      <button @click="$emit('dismiss')" type="button"
        class="hidden md:flex absolute top-4 right-4 z-10 w-9 h-9 items-center justify-center rounded-full bg-[#F2F2F7] text-[#8E8E93] hover:bg-[#E5E5EA] hover:text-[#1C1C1E] transition-colors cursor-pointer">
        <span class="material-symbols-outlined notranslate text-[20px]">close</span>
      </button>

      <!-- Scrollable content -->
      <div class="overflow-y-auto px-6 pb-8 -webkit-overflow-scrolling:touch md:flex-1 md:min-h-0 md:px-0 md:pb-0">
        ...
        <template v-if="step === 1">
          <div class="md:flex md:items-stretch">
            ...
            <div
              class="hidden md:flex md:flex-col md:justify-between md:w-[280px] lg:w-[320px] shrink-0 bg-gradient-to-br from-orange-500 to-orange-600 text-white p-8 lg:p-9">
              <div>
                <div class="flex items-center gap-2.5">
                  <div class="w-10 h-10 rounded-2xl bg-white/15 flex items-center justify-center">
                    <span class="material-symbols-outlined notranslate text-[22px]">qr_code_2</span>
                  </div>
                  <span class="text-lg font-bold tracking-tight">Ubiqueme</span>
                </div>
                <h3 class="mt-8 text-[22px] lg:text-2xl font-bold leading-snug">Notificaciones al instante</h3>
                <p class="mt-3 text-[13px] leading-relaxed opacity-90">
                  Conecta tu WhatsApp y entérate al momento cada vez que alguien escanee tu código QR.
                </p>
                <ul class="mt-7 space-y-4">
                  <li class="flex items-center gap-3 text-[13px]">
                    <span class="material-symbols-outlined notranslate text-[18px] text-orange-200">bolt</span>
                    Alertas en tiempo real
                  </li>
                  <li class="flex items-center gap-3 text-[13px]">
                    <span class="material-symbols-outlined notranslate text-[18px] text-orange-200">verified_user</span>
                    Verificación segura por WhatsApp
                  </li>
                  <li class="flex items-center gap-3 text-[13px]">
                    <span class="material-symbols-outlined notranslate text-[18px] text-orange-200">shield</span>
                    Tu número solo para notificaciones
                  </li>
                </ul>
              </div>
              <p class="mt-8 text-[11px] opacity-70">Ubiqueme.com</p>
            </div>

            <!-- Right content -->
            <div class="md:flex-1 md:p-8 lg:p-10">
              <!-- Header -->
              <div class="flex items-start gap-3.5 pt-1 pb-5 md:pt-0 md:pb-6">
                <div class="flex h-11 w-11 items-center justify-center rounded-full bg-orange-100 shrink-0 mt-0.5">
                  <svg class="w-5.5 h-5.5 text-orange-500" style="width:22px;height:22px" viewBox="0 0 24 24"
                    fill="currentColor">
                    <path
                ...
                <div class="flex-1 min-w-0">
                  <h2 class="text-[17px] font-semibold text-[#1C1C1E] leading-tight md:text-xl">
                    ¡Bienvenido a <span class="text-orange-500">Ubiqueme</span>!
                  </h2>
                  <p class="mt-1 text-[13px] text-[#8E8E93] leading-relaxed md:text-[14px]">
              ...
              <div class="mb-5 px-3.5 py-3 rounded-xl bg-[#F2F2F7] md:px-4 md:py-4">
                <div class="flex items-start gap-2.5">
                  ...
                  <div>
                    <p class="text-[11px] font-semibold text-[#3A3A3C] uppercase tracking-wider">Verificación de
                      seguridad</p>
                    <p class="text-[12px] text-[#8E8E93] leading-relaxed mt-0.5 md:text-[13px]">Le enviaremos un código
                      por WhatsApp para confirmar que es el dueño de este número. Así protegemos su cuenta.</p>
                  </div>
                </div>
              </div>
              ...
              <div class="space-y-4 md:space-y-5">
                <!-- Phone input group -->
                <div>
                  <label class="text-[13px] font-medium text-[#3A3A3C] mb-1.5 block md:text-[14px]">Número de
                    WhatsApp</label>
                  <div class="flex gap-2">
                    ...
                    <div class="relative shrink-0">
                      <input v-model="countryCodeSearch" type="text" inputmode="numeric" placeholder="+52"
                        ...
                        class="w-[100px] h-11 px-3 rounded-2xl bg-[#F2F2F7] text-[#1C1C1E] text-[15px] font-medium text-center outline-none md:w-[120px] md:h-12 md:text-[16px]"
                        v-if="!selectedCountry || editingCountry" />

                      <button v-else @click="editCountry" type="button"
                        class="flex items-center gap-1 h-11 px-3 rounded-2xl bg-[#F2F2F7] text-[#1C1C1E] text-[15px] font-medium cursor-pointer transition-all active:bg-[#E5E5EA] md:h-12 md:px-4 md:text-[16px]">
                    ...
                    <div class="relative flex-1">
                      <input ref="phoneInput" v-model="phoneNumber" type="tel" placeholder="55 1234 5678"
                        ...
                        class="w-full h-11 px-4 rounded-2xl bg-[#F2F2F7] text-[#1C1C1E] text-[15px] font-medium tracking-wider outline-none placeholder:text-[#8E8E93] disabled:opacity-40 disabled:cursor-not-allowed md:h-12 md:text-[16px]" />
                    </div>
                  </div>
                ...
                <button @click="handleSendOtp" :disabled="isSendingOtp || !isValid"
                  class="w-full h-11 rounded-full bg-orange-500 text-white text-[15px] font-semibold hover:bg-orange-600 active:scale-[0.97] transition-all duration-150 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer md:h-12 md:text-[16px] md:shadow-lg md:shadow-orange-500/25">
        ...
        <template v-else-if="step === 2">
          <div class="md:flex md:items-stretch">
            ...
            <div
              class="hidden md:flex md:flex-col md:justify-between md:w-[280px] lg:w-[320px] shrink-0 bg-gradient-to-br from-emerald-500 to-emerald-600 text-white p-8 lg:p-9">
              <div>
                <div class="flex items-center gap-2.5">
                  <div class="w-10 h-10 rounded-2xl bg-white/15 flex items-center justify-center">
                ...
                <h3 class="mt-8 text-[22px] lg:text-2xl font-bold leading-snug">Verifique su número</h3>
                <p class="mt-3 text-[13px] leading-relaxed opacity-90">
                  Enviamos un código de 6 dígitos a su WhatsApp. Ingrese el código para confirmar.
                </p>
                <div class="mt-7 flex items-center gap-3 rounded-2xl bg-white/10 p-4">
                  <span class="material-symbols-outlined notranslate text-[20px]">smartphone</span>
                  <div class="min-w-0">
                    <p class="text-[11px] opacity-80">Número a verificar</p>
                    <p class="text-[15px] font-bold truncate">{{ selectedCountry.prefix }} {{ phoneNumber }}</p>
                  </div>
                </div>
              </div>
              <p class="mt-8 text-[11px] opacity-70">Verificación OTP</p>
            </div>
            ...
            <div class="md:flex-1 md:p-8 lg:p-10">
              ...
              <div class="flex items-start gap-3.5 pt-1 pb-5 md:pt-0 md:pb-6">
                ...
                <div class="flex-1 min-w-0">
                  <h2 class="text-[17px] font-semibold text-[#1C1C1E] leading-tight md:text-xl">Verifique su número</h2>
                  <p class="mt-1 text-[13px] text-[#8E8E93] leading-relaxed md:text-[14px]">
              ...
              <div class="space-y-4 md:space-y-5">
                <div>
                  <label class="text-[13px] font-medium text-[#3A3A3C] mb-1.5 block md:text-[14px]">Código de
                    verificación</label>
                  <input v-model="otpCode" type="text" inputmode="numeric" maxlength="6" placeholder="••••••"
                    @input="onOtpInput"
                    class="w-full h-11 px-4 rounded-2xl bg-[#F2F2F7] text-[#1C1C1E] text-[20px] font-semibold tracking-[0.4em] text-center outline-none placeholder:text-[#C6C6C8] md:h-12 md:text-[24px]"
                    autofocus />
                </div>
                ...
                <div class="flex flex-col items-center gap-2 pt-1">
                  ...
                  <button @click="handleResendOtp" :disabled="isSendingOtp || resendCooldown > 0"
                    class="h-9 px-5 rounded-full bg-[#F2F2F7] text-[#1C1C1E] text-[13px] font-semibold hover:bg-[#E5E5EA] active:scale-[0.97] transition-all duration-150 disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer md:h-10">
                    Enviar de nuevo
        ...
        <!-- Footer -->
        <p class="mt-6 text-[11px] text-[#8E8E93] text-center leading-relaxed md:px-8 md:pb-8">
...
@media (min-width: 768px) {
  .animate-ios-sheet {
    animation: iosDialog 0.3s cubic-bezier(0.16, 1, 0.3, 1) forwards;
  }
}
