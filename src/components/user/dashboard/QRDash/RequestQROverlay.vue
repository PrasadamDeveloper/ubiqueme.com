...
<template>
  <div v-if="visible" class="fixed inset-0 bg-gray-900/60 flex items-center justify-center z-100 p-4"
    @click.self="handleClose">
    <div v-if="visible && subscription"
      class="relative w-full max-w-5xl bg-white border border-slate-200 rounded-3xl overflow-hidden font-google-sans">
      ...
      <button @click="handleClose"
        class="absolute top-4 right-4 z-20 w-9 h-9 rounded-xl bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-slate-200 hover:text-slate-700 transition-all cursor-pointer">
      ...
      <div class="relative z-10 p-6 md:p-8 max-h-[85vh] overflow-y-auto">
        ...
        <div class="mb-6">
          <div
            class="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-orange-200 bg-orange-50 text-orange-600 text-[10px] font-black uppercase tracking-[0.2em] mb-3">
          ...
          <p class="text-slate-500 mt-1 text-sm max-w-xl leading-relaxed">
        ...
        <div class="mb-6 p-4 rounded-2xl border border-slate-200 bg-slate-50">
          <div class="flex items-center gap-3">
            ...
            <div class="ml-auto text-right">
              <p class="text-xs text-slate-500">Envíos usados</p>
        ...
        <div class="mb-6 p-4 rounded-2xl border border-slate-200 bg-slate-50">
          <div class="flex items-center gap-2 mb-4">
            <span class="material-symbols-outlined notranslate text-[16px] text-slate-400">location_on</span>
            <label class="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Dirección de entrega</label>
          </div>
          <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
            ...
            <div>
              <label class="text-[9px] font-bold uppercase tracking-wider text-slate-500 ml-1 mb-1 block">Código Postal
                *</label>
          ...
          <div class="mb-3">
            <label class="text-[9px] font-bold uppercase tracking-wider text-slate-500 ml-1 mb-1 block">Teléfono de
              contacto
              *</label>
          ...
          <div>
            <label class="text-[9px] font-bold uppercase tracking-wider text-slate-500 ml-1 mb-1 block">Notas adicionales
              <span class="text-slate-300 normal-case">(opcional)</span></label>
        ...
        <div class="mb-6">
          <div class="flex items-center justify-between mb-3">
            <label class="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">
              ...
              <span class="text-slate-300 ml-2">({{ selectedQRIds.size }} seleccionados)</span>
            </label>
            <div class="flex gap-2">
              <button @click="selectAll"
                class="text-[10px] font-bold uppercase tracking-wider px-3 py-1 rounded-lg border border-slate-200 text-slate-500 hover:text-slate-700 hover:border-slate-300 transition-all cursor-pointer">
                Seleccionar todos
              </button>
          ...
          <div class="space-y-3 pr-1 scrollbar-thin">
            <div v-for="qr in qrs" :key="qr.id" class="rounded-xl border transition-all duration-200 overflow-hidden"
              ...
              <div class="flex items-center gap-3 p-3 cursor-pointer"
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2">
                    <span class="text-sm font-bold text-slate-900 truncate">{{ qr.name }}</span>
                  ...
                  <div class="flex items-center gap-3 mt-0.5">
                    <span class="text-[10px] text-slate-400 font-mono">#{{ qr.id }}</span>
                    <span class="text-[10px] text-slate-400 capitalize">{{ qr.category || 'general' }}</span>
                ...
                <span
                  class="material-symbols-outlined notranslate text-slate-400 text-[18px] transition-transform duration-200 shrink-0"
              ...
              <div v-if="expandedQR === qr.id" class="border-t border-slate-200 px-3 py-4 space-y-4">
                <!-- Formato -->
                <div class="space-y-2">
                  <label class="text-[9px] font-black uppercase tracking-[0.2em] text-slate-500 ml-1">
                  ...
                  <div class="grid grid-cols-2 gap-2">
                    <button @click="setLayout(qr.id, 'compact')"
                      class="relative rounded-xl border p-3 text-left transition-all cursor-pointer" :class="getQrCust(qr.id).layout === 'compact'
                        ? 'border-orange-500/40 bg-orange-50 shadow-[0_0_15px_rgba(249,115,22,0.1)]'
                        : 'border-slate-200 bg-white hover:bg-slate-50'">
                      <div class="flex items-center justify-between">
                        <p class="text-xs font-bold text-slate-900">Compacto</p>
                        <span class="material-symbols-outlined notranslate text-[16px]"
                          :class="getQrCust(qr.id).layout === 'compact' ? 'text-orange-500' : 'text-slate-300'">qr_code_2</span>
                      </div>
                      <p class="text-[9px] text-slate-500 mt-0.5">Solo el código QR</p>
                    </button>
                    <button @click="setLayout(qr.id, 'detail')"
                      ...
                      <div class="flex items-center justify-between">
                        ...
                        <span class="material-symbols-outlined notranslate text-[16px]"
                          :class="getQrCust(qr.id).layout === 'detail' ? 'text-orange-500' : 'text-slate-300'">article</span>
                      </div>
                      <p class="text-[9px] text-slate-500 mt-0.5">Nombre + QR</p>
                    </button>
                  </div>
                </div>
                  ...
                  <div class="grid grid-cols-4 gap-2">
                    <button v-for="size in sizeOptions" :key="size" @click="setSize(qr.id, size)"
                      ...
                      :class="getQrCust(qr.id).size === size
                        ? 'border-orange-500/40 bg-orange-50 text-orange-600 shadow-[0_0_12px_rgba(249,115,22,0.12)]'
                  ...
                  <div class="grid grid-cols-2 gap-2">
                    <button @click="setGluePosition(qr.id, 'frontal')"
                      class="relative rounded-xl border p-3 text-left transition-all cursor-pointer" :class="getQrCust(qr.id).gluePosition === 'frontal'
                        ? 'border-blue-500/40 bg-blue-50 shadow-[0_0_15px_rgba(59,130,246,0.1)]'
                        : 'border-slate-200 bg-white hover:bg-slate-50'">
                      <div class="flex items-center justify-between">
                        <p class="text-xs font-bold text-slate-900">Frontal</p>
                        <span class="material-symbols-outlined notranslate text-[16px]"
                          :class="getQrCust(qr.id).gluePosition === 'frontal' ? 'text-blue-600' : 'text-slate-300'">flip_to_front</span>
                      </div>
                      <p class="text-[9px] text-slate-500">Para pegar por dentro de cristales</p>
                    </button>
                    <button @click="setGluePosition(qr.id, 'trasero')"
                      ...
                      <div class="flex items-center justify-between">
                        ...
                        <span class="material-symbols-outlined notranslate text-[16px]"
                          :class="getQrCust(qr.id).gluePosition === 'trasero' ? 'text-blue-600' : 'text-slate-300'">flip_to_back</span>
                      </div>
                      <p class="text-[9px] text-slate-500">Etiqueta tradicional</p>
                    </button>
                  </div>
                </div>
              </div>
            ...
            <div v-if="qrs.length === 0" class="text-center py-8 text-slate-400 text-sm">
        ...
        <!-- Actions -->
          <div
            class="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between pt-4 border-t border-slate-200">
            <div class="text-xs text-slate-500">
              <span class="font-bold text-slate-700">{{ selectedQRIds.size }}</span> código(s) seleccionado(s)
          ...
          <div class="flex gap-3">
            <button @click="handleClose"
              class="px-6 py-2.5 rounded-xl border border-slate-200 text-slate-600 text-sm font-medium hover:bg-slate-50 hover:text-slate-900 transition-all cursor-pointer">
              Cancelar
            </button>
