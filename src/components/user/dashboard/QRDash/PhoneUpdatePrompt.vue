<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-gray-900/50"
    ...
    <div
      class="relative w-full max-w-lg bg-white border border-slate-200 rounded-3xl p-6 sm:p-10 animate-fade-in overflow-hidden">
       <button @click="$emit('dismiss')"
        class="absolute flex items-center gap-1 top-2 left-5 border border-orange-200 animate-fade-right p-1 rounded-md font-google-sans text-orange-600 text-sm hover:bg-orange-500 hover:text-white cursor-pointer">
      ...
      <template v-if="step === 1">
        ...
        <div class="flex items-start gap-4 mb-8">
          <div
            class="flex items-center justify-center w-12 h-12 border shrink-0 bg-orange-50 border-orange-200 rounded-2xl">
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="mt-1 text-sm font-medium leading-relaxed text-slate-500">
              Ingrese el nuevo número donde recibirá las notificaciones de escaneo de sus códigos QR.
            </p>
          </div>
        </div>
        ...
        <div class="space-y-5">
            ...
            <div class="flex gap-2">
              ...
              <div class="relative shrink-0">
                ...
                <input v-model="countryCodeSearch" type="text" inputmode="numeric" placeholder="+52"
                  ...
                  class="w-[110px] h-14 px-3 bg-white border border-slate-200 hover:border-slate-300 rounded-2xl text-slate-900 text-sm font-medium text-center focus:border-orange-500 focus:outline-none focus:bg-orange-50 transition-all placeholder:text-slate-400"
                  v-if="!selectedCountry || editingCountry" />

                <!-- Chip when country is selected -->
                <button v-else @click="editCountry" type="button"
                  class="flex items-center gap-1.5 h-14 px-3 bg-white border border-slate-200 hover:border-slate-300 rounded-2xl text-slate-900 text-sm font-medium cursor-pointer transition-all hover:bg-slate-50">
                  <span class="text-lg">{{ selectedCountry.flag }}</span>
                  <span class="font-medium tracking-wider">{{ selectedCountry.prefix }}</span>
                  <span class="text-xs text-slate-400 material-symbols-outlined notranslate">expand_more</span>
                </button>

                <!-- Suggestions dropdown -->
                <div v-if="suggestionsOpen && filteredCountries.length"
                  class="absolute z-50 mt-1 left-0 min-w-[240px] bg-white border border-slate-200 rounded-2xl shadow-xl overflow-hidden max-h-48 overflow-y-auto">
                  <button v-for="c in filteredCountries" :key="c.code" @mousedown.prevent="selectCountry(c)"
                    class="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-slate-50 transition-colors text-slate-700 text-sm text-left">
                    <span class="text-lg shrink-0">{{ c.flag }}</span>
                    <span class="font-medium truncate">{{ c.name }}</span>
                    <span class="ml-auto text-slate-400 shrink-0">{{ c.prefix }}</span>
                  </button>
                </div>
              </div>
              ...
              <div class="relative flex-1">
                <input ref="phoneInput" v-model="phoneNumber" type="tel" placeholder="55 1234 5678"
                  ...
                  class="w-full px-5 text-lg font-medium tracking-wider text-slate-900 transition-all border h-14 bg-white border-slate-200 hover:border-slate-300 rounded-2xl placeholder:text-slate-400 focus:border-orange-500 focus:outline-none focus:bg-orange-50 disabled:opacity-40 disabled:cursor-not-allowed" />
              </div>
            </div>
            <p v-if="selectedCountry" class="text-[11px] text-slate-400 font-medium ml-1">
          ...
          <button @click="handleSendOtp" :disabled="isSendingOtp || !isValid"
            class="group w-full h-14 bg-orange-500 text-white rounded-2xl font-black text-xs uppercase tracking-widest transition-all duration-300 hover:bg-orange-600 hover:shadow-[0_0_20px_rgba(249,115,22,0.4)] active:scale-[0.98] flex items-center justify-center gap-3 disabled:opacity-50 disabled:grayscale disabled:cursor-not-allowed">
            ...
            <span v-else class="flex items-center gap-2">
              <span class="w-5 h-5 border-2 rounded-full border-white/30 border-t-white animate-spin"></span>
              Enviando...
            </span>
          </button>
        </div>
      ...
      <template v-else-if="step === 2">
        ...
        <div class="flex items-start gap-4 mb-8">
          <div
            class="flex items-center justify-center w-12 h-12 border shrink-0 bg-green-50 border-green-200 rounded-2xl">
            <span class="text-3xl text-green-500 material-symbols-outlined notranslate">lock</span>
          </div>
          <div class="flex-1 min-w-0">
            <h2 class="text-xl font-black tracking-tight text-slate-900 sm:text-2xl">
              Verifique su nuevo número
            </h2>
            <p class="mt-1 text-sm font-medium leading-relaxed text-slate-500">
              Hemos enviado un código de verificación de 6 dígitos al nuevo número
              <span class="font-bold text-slate-700">{{ selectedCountry.prefix }} {{ phoneNumber }}</span>
        ...
        <div class="space-y-5">
          <div class="space-y-2">
            ...
            <input v-model="otpCode" type="text" inputmode="numeric" maxlength="6" placeholder="••••••"
              @input="onOtpInput"
              class="w-full h-14 px-5 bg-white border border-slate-200 hover:border-slate-300 rounded-2xl text-slate-900 placeholder:text-slate-400 focus:border-orange-500 focus:outline-none focus:bg-orange-50 transition-all tracking-[0.5em] text-center text-2xl font-black"
              autofocus />
          </div>
          ...
          <div class="text-center flex flex-col items-center gap-2.5">
            ...
            <button @click="handleResendOtp" :disabled="isSendingOtp"
              class="text-xs transition-colors text-slate-400 disabled:opacity-30 group">
              <span
                class="group-hover:text-slate-700 group-hover:ring group-hover:ring-slate-200 shadow-sm group-hover:shadow transition-shadow duration-400 ease-out px-1 py-1.5 rounded-lg border border-slate-200 bg-white cursor-pointer font-google-sans font-medium">
                Enviar de nuevo
              </span>
            </button>
            <span v-if="resendCooldown > 0" class="ml-2 text-xs font-medium text-slate-300">
              ({{ resendCooldown }}s)
            </span>
          </div>
          ...
          <div class="text-center">
            <button @click="handleCancelOTPValidation" :disabled="isSendingOtp"
              class="text-xs font-medium transition-colors text-orange-600/60 italic hover:text-slate-700 disabled:opacity-30 flex items-center justify-center w-full gap-1 cursor-pointer">
      ...
      <p class="mt-5 text-[10px] text-slate-400 font-medium text-center leading-relaxed">
        El nuevo número solo se usará para enviarle notificaciones de escaneo de sus códigos QR.
      </p>
    </div>
  </div>
</template>
...
const handleVerifyOtp = async () => {
  ...
  try {
    ...
    toast.success(`Número actualizado exitosamente. Ahora notificaciones en ${phone}`)
    emit('dismiss')
  } catch (error: unknown) {
    const err = error as { message?: string }
    toast.error(`Error de conexión: ${err.message || 'Desconocido'}`)
...
const handleCancelOTPValidation = async () => {
  try {
    ...
    if (response.ok) {
      toast.info('Código OTP cancelado')
      step.value = 1;
      phoneNumber.value = ''
    }
  } catch (error) {
