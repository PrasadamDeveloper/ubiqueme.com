<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-[#070b14]/80"
    ...
    <div
      class="relative w-full max-w-lg bg-[#0c0c0c] border border-white/10 rounded-3xl p-6 sm:p-10 animate-fade-in overflow-hidden">
       <button @click="$emit('dismiss')"
        class="absolute flex items-center gap-1 top-2 left-5 border border-orange-950 animate-fade-right p-1 rounded-md font-google-sans text-orange-200 text-sm hover:bg-orange-600 hover:text-white cursor-pointer">
        ...
        <div class="space-y-5">
          ...
          <div class="text-center flex flex-col items-center gap-2.5">
            ...
            <button @click="handleResendOtp" :disabled="isSendingOtp"
              ...
              <span
                class="group-hover:text-slate-200 group-hover:ring group-hover:ring-offset-white shadow-white group-hover:shadow-sm transition-shadow duration-400 ease-out px-1 py-1.5 rounded-lg border border-gray-500 bg-[#0F0F0F] cursor-pointer font-google-sans font-medium">
                Enviar de nuevo
              </span>
            </button>
